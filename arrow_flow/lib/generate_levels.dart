...
/// 100% SOLID LABYRINTH SERPENTINE GENERATOR (v57)
/// Features:
/// - Every 5th level (L5, L10, L15, L20, ...) is a SUPER ULTRA HARD BOSS LEVEL!
/// - 98-100% solid coverage, zero gaps, long labyrinth arrows, 100% solvable.

void main() {
  final rng = Random(443322);
  final levels = <Level>[];

  print('Generating 500 100% SOLID LABYRINTH levels with BOSSES every 5 levels (v57)...');
  print('');
  ...
  for (int id = 1; id <= 500; id++) {
    final (diff, gW, gH) = _getGridParamsForLevel(id);

    Level level;
    while (true) {
      level = _generateLabyrinthLevel(id, diff, gW, gH, rng);
    ...
    if (id <= 15 || id % 5 == 0 || id == 21 || id == 51 || id == 101 || id == 201 || id == 500) {
      ...
      final isBoss = id % 5 == 0 ? ' [🔥 BOSS 🔥]' : '';
      print('L$id ($diff)$isBoss [${gW}x${gH}]: ${level.arrows.length} arrows (avg ${avgLen.toStringAsFixed(1)} cells), $coverage% fill [Solvable ✓]');
    }
  }

  stopwatch.stop();
  print('');
  print('Generated 500 levels in ${stopwatch.elapsed.inMilliseconds} ms (Rejected: $rejectedCount)');

  final json = const JsonEncoder.withIndent('  ')
      .convert(levels.map((l) => l.toJson()).toList());
  File('assets/levels/levels_pack_001.json').writeAsStringSync(json);
  print('Saved to assets/levels/levels_pack_001.json');
}

(String, int, int) _getGridParamsForLevel(int id) {
  final isBoss = id % 5 == 0;
